package com.smd.gctcore.integration.mmce.adapter.draconicevolution;
...
/**
 * 龙之研究觉醒聚合合成适配器 (Tier 2 - Awakened/Draconic)
 */
public class AdapterDEAwakenedFusionCrafting extends RecipeAdapter {

    private static final int BASE_ENERGY_PER_TICK = 3000;

    public AdapterDEAwakenedFusionCrafting() {
        super(new ResourceLocation("draconicevolution", "fusion_crafting_awakened"));
    }
    ...
    public Collection<MachineRecipe> createRecipesFor(ResourceLocation owningMachineName,
        ...
        for (IFusionRecipe fusionRecipe : fusionRecipes) {
            try {
                // 只处理觉醒等级 (Tier 2)
                if (fusionRecipe.getRecipeTier() != 2) {
                    continue;
                }
    ...
    private MachineRecipe createMachineRecipe(IFusionRecipe fusionRecipe, ResourceLocation owningMachineName, List<RecipeModifier> modifiers) {
        ...
        MachineRecipe recipe = createRecipeShell(
                new ResourceLocation("draconicevolution", "fusion_awakened_" + incId),
                owningMachineName,
                processingTime,
                incId++,
                false
    ...
    private int calculateProcessingTime(long energyCost, int ingredientCount, int tier, List<RecipeModifier> modifiers) {
        // 觉醒等级处理时间较长
        float baseTime = (energyCost / 1000.0f + ingredientCount * 12.0f);

        // 限制最小和最大时间
        baseTime = Math.max(40, Math.min(baseTime, 1200)); // 2秒到1分钟
    ...
    private int calculateEnergyPerTick(long energyCost, int ingredientCount, int tier, List<RecipeModifier> modifiers) {
        ...
        // 根据总能耗调整
        if (totalEnergy > 2000000) { // 如果总能耗很高，增加每tick消耗
            baseEnergyPerTick = (int) Math.min(baseEnergyPerTick * 2.2, totalEnergy / 100);
        }
