...
/**
 * 龙之研究飞龙聚合合成适配器 (Tier 1 - Wyvern)
 */
public class AdapterDEWyvernFusionCrafting extends RecipeAdapter {

    private static final int BASE_ENERGY_PER_TICK = 1500;

    public AdapterDEWyvernFusionCrafting() {
        super(new ResourceLocation("draconicevolution", "fusion_crafting_wyvern"));
    }
    ...
    public Collection<MachineRecipe> createRecipesFor(ResourceLocation owningMachineName,
        ...
        for (IFusionRecipe fusionRecipe : fusionRecipes) {
            try {
                // 只处理飞龙等级 (Tier 1)
                if (fusionRecipe.getRecipeTier() != 1) {
                    continue;
                }
    ...
    private MachineRecipe createMachineRecipe(IFusionRecipe fusionRecipe, ResourceLocation owningMachineName, List<RecipeModifier> modifiers) {
        ...
        MachineRecipe recipe = createRecipeShell(
                new ResourceLocation("draconicevolution", "fusion_wyvern_" + incId),
                owningMachineName,
                processingTime,
                incId++,
                false
    ...
    private int calculateProcessingTime(long energyCost, int ingredientCount, int tier, List<RecipeModifier> modifiers) {
        // 飞龙等级处理时间中等
        float baseTime = (energyCost / 1500.0f + ingredientCount * 8.0f);

        // 限制最小和最大时间
        baseTime = Math.max(20, Math.min(baseTime, 800)); // 1秒到40秒
    ...
    private int calculateEnergyPerTick(long energyCost, int ingredientCount, int tier, List<RecipeModifier> modifiers) {
        ...
        // 根据总能耗调整
        if (totalEnergy > 1000000) { // 如果总能耗很高，增加每tick消耗
            baseEnergyPerTick = (int) Math.min(baseEnergyPerTick * 1.8, totalEnergy / 120);
        }
